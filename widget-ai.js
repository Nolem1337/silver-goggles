 ...
 *
 * No API key in code — user enters it once in the widget.
 * Key is stored safely in the browser (localStorage).
 */

(function () {

  // ============================================================
  //  CONFIG — edit for each client (5 minutes), no API key needed
  // ============================================================
  const CONFIG = {
...
- Never make up information`,
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      #aiw-key-screen {
        padding:20px 16px; display:flex; flex-direction:column; gap:12px; flex:1;
      }
  ...
  function buildWidget() {
    ...
    win.innerHTML = `
      ...
      <div id="aiw-body" style="display:flex;flex-direction:column;flex:1;overflow:hidden;">
      </div>
    ...
    function showKeyScreen() {
      bodyEl.innerHTML = `
        <div id="aiw-key-screen">
          <p><strong>Enter your Anthropic API key</strong> to activate the AI assistant.<br>Your key is stored only in this browser.</p>
          <input type="password" id="aiw-key-input" placeholder="sk-ant-api03-..." />
          <p id="aiw-key-error">Invalid key — please check and try again.</p>
          <button id="aiw-key-btn">Activate AI</button>
          <p style="font-size:11px;color:#aaa;">Get your free key at console.anthropic.com</p>
        </div>
      `;
      document.getElementById("aiw-key-btn").onclick = async () => {
    ...
    function showChatScreen() {
      ...
      async function handleSend(text) {
        ...
        try {
          ...
          removeTyping();
          if (apiKey) addMessage("Sorry, something went wrong. Please try again.", "bot");
        } finally {
          setLoading(false);
        }
      }
    ...
    launcher.onclick = () => {
      ...
      if (isOpen) setTimeout(() => {
        const inp = document.getElementById("aiw-input") || document.getElementById("aiw-key-input");
        if (inp) inp.focus();
      }, 300);
    };
  }
